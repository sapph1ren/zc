...
nk_gdifont_create(const char *name, int size) {
    ...
    /* 
       1. Используем CreateFontW для поддержки Юникода.
       2. Преобразуем имя шрифта из UTF-8 в WideChar (L"Name").
    */
    wchar_t wname[64];
    MultiByteToWideChar(CP_UTF8, 0, name, -1, wname, 64);

    font->handle = CreateFontW(
        -size,                  // Отрицательное значение для точного размера в пикселях
        0, 0, 0, 
        FW_NORMAL, FALSE, FALSE, FALSE, 
        RUSSIAN_CHARSET,        // КРИТИЧНО: поддержка кириллицы
        OUT_DEFAULT_PRECIS, 
        CLIP_DEFAULT_PRECIS, 
        CLEARTYPE_QUALITY,      // Хорошее сглаживание
    ...
    font->height = (int)metric.tmHeight;
